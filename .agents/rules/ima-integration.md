...
3. **基于已有文档设计用例与增量差异（Diff）分析**：
   ...
   - **【回归用例必须实体化生成导出（强制）】**：在差异比对中识别出的 🔵 需重点回归的存量范围，**必须作为实体测试用例完整生成并输出**到导出的 XMind 测试资产（`.xmind`）中（在 XMind 中作为各模块回归分支或【存量重点回归测试】专属分支），绝不能只停留在评审摘要中。
   - 在需求分析摘要中明确标注：“输入来源：IMA 知识库 - <条目名称>”。
4. **严格清理中间产物**：
   - 解析在线原型（如 Mockplus）产生的抓取缓存（如 `exports/mockplus`）属于调试提取中间物。
   - 分析提取及用例导出完成后，必须在当前轮次结束前立即自动删除，确保 `exports/` 仅保留最终的 `.xmind` 产物。
